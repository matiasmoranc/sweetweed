...
function ver(i) {
    var detalles = document.querySelectorAll('.producto__info-producto');
    var botones = document.querySelectorAll('.producto__indice')[0].children;
    var items = document.querySelectorAll('.producto__item');
    var cantidad = items.length;
    var anchoPantalla = document.querySelectorAll('body')[0].clientWidth;

    console.log(cantidad);

    for (let b = 0; b < cantidad; b++) {
        if(anchoPantalla > 1000) {
            items[b].setAttribute('style', 'transform:translateY(' + (i * (-500)) + 'px)');
        } else {
            items[b].setAttribute('style', 'transform:translateY(' + (i * (-200)) + 'px)');
        }
        detalles[b].classList.add('producto__info-producto--ocultar');     
        if (botones[b].children[0].classList.contains('checked-active')) {
            botones[b].children[0].classList.remove('checked-active');     
            botones[b].children[0].src = './img/circle.webp';  
        }

    }
    setTimeout(() => {
        detalles[i].classList.remove('producto__info-producto--ocultar');
    }, 500);
    botones[i].children[0].classList.add('checked-active');
    botones[i].children[0].src = './img/checked.webp'; 
